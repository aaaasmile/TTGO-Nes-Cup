...
int state_save(void) {
  ...
  nofrendo_log_printf("avoid save state!!\n");
  return 0;
...
_error:
  gui_sendmsg(GUI_RED, "state save - error: %s", SNSS_GetErrorString(status));
  SNSS_CloseFile(&snssFile);
  return -1;
}

#define MYASSERT(expr) nofrendo_log_assert((int)(expr), __LINE__, __FILE__, NULL)
#define MYASSERT_MSG(msg) nofrendo_log_assert(false, __LINE__, __FILE__, (msg))
...
int state_load(void) {
  ...
  machine = nes_getcontextptr();
  MYASSERT(machine);
  nofrendo_log_printf("load state - Machine is ok\n");

  nofrendo_log_printf("avoid load state!!\n");
  return 0;

  /* build the state name using the ROM's name and the slot number */
  strncpy(fn, machine->rominfo->filename, PATH_MAX - 4);

  MYASSERT(state_slot >= FIRST_STATE_SLOT && state_slot <= LAST_STATE_SLOT);
  sprintf(ext, ".ss%d", state_slot);
  osd_newextension(fn, ext);
  nofrendo_log_printf("load state - new extension %s ext is %s\n", fn, ext);

  /* open our file for writing */
  status = SNSS_OpenFile(&snssFile, fn, SNSS_OPEN_READ);
  if (SNSS_OK != status){
     nofrendo_log_printf("load state - openfile error %s\n", fn);
     goto _error;
  }
  nofrendo_log_printf("load state - file open ok");  
  ...
  for (i = 0; i < snssFile->headerBlock.numberOfBlocks; i++) {
    ...
    if (SNSS_OK != status){
       nofrendo_log_printf("load state - SNSS_GetNextBlockType error\n");  
       goto _error;
    }
    ...
    if (SNSS_OK != status){
       nofrendo_log_printf("load state - SNSS_ReadBlock error\n");  
       goto _error;
    }
  ...
  status = SNSS_CloseFile(&snssFile);
  if (SNSS_OK != status){
     nofrendo_log_printf("load state - SNSS_CloseFile error\n"); 
     goto _error;
  }
...
_error:
  gui_sendmsg(GUI_RED, "state_load error: %s", SNSS_GetErrorString(status));
  SNSS_CloseFile(&snssFile);
  return -1;
}
